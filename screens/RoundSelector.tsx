...
const RoundSelector: React.FC = () => {
  ...
  const selectedRounds = useSelector((state: RootState) => state.player.gameRound);
  ...
  return (
    <View className="w-full">
      ...
      <View className="flex-row items-center mb-4 px-2">
        ...
        <Text style={{ fontSize: rf(1.1) }} className="text-white/50 font-black uppercase tracking-[4px]">
          Mission Rounds
        </Text>
      </View>
      ...
      <View className="bg-white/[0.03] border border-white/10 rounded-[32px] p-4 flex-row flex-wrap justify-between">
        {roundOptions.map((round) => {
          ...
          return (
            <Pressable
              key={`round-${round}`}
              onPress={() => dispatch(setGameRound(round))}
              // We use style for the background color to avoid string interpolation crashes in NativeWind v4
              style={{
                width: '18%',
                aspectRatio: 1,
      ...
      <View className="mt-4 bg-indigo-500/10 self-center px-4 py-1.5 rounded-full border border-indigo-500/20">
        <Text style={{ fontSize: rf(0.9) }} className="text-indigo-300 font-bold uppercase tracking-[2px]">
