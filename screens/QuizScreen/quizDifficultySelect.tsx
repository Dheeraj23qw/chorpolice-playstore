...
const StartButton = memo(({ label, onPress }: { label: string; onPress: () => void }) => {
  return (
...
const DifficultyCard = memo(({ option, selected, onSelect }: any) => {
  const config = {
    easy: { icon: "leaf-outline", color: "#22c55e", desc: "A relaxed pace for everyone" },
    medium: { icon: "thunderstorm-outline", color: "#f59e0b", desc: "Test your limits" },
    hard: { icon: "skull-outline", color: "#ef4444", desc: "Only for the true experts" },
      ...
      >
        <View className="flex-row items-center justify-between">
          <View className="flex-row items-center flex-1">
            ...
            >
              <Ionicons name={config.icon as any} size={rf(3.5)} color={selected ? "white" : "rgba(255,255,255,0.4)"} />
            </View>

            <View className="flex-1">
              <Text style={{ fontSize: rf(2.4) }} className={`font-bold ${selected ? "text-white" : "text-white/70"}`}>
                {option.charAt(0).toUpperCase() + option.slice(1)}
              </Text>
              <Text style={{ fontSize: rf(1.6) }} className="text-white/40 mt-1" numberOfLines={1}>
                {config.desc}
              </Text>
            </View>
          </View>
          ...
          >
            {selected && <Ionicons name="checkmark" size={rf(1.6)} color="white" />}
          </View>
        </View>
      </View>
    </Pressable>
...
const QuizDifficultyScreen = () => {
  ...
  const handleSelect = useCallback((option: DifficultyOption) => {
  ...
  return (
    <View className="flex-1 bg-[#09090b]">
      {/* Background Decor */}
      <View style={{ width: wp(80), height: wp(80), top: -hp(10), right: -wp(20) }} className="absolute bg-indigo-600/20 rounded-full blur-3xl" />
      <View style={{ width: wp(60), height: wp(60), top: hp(40), left: -wp(20) }} className="absolute bg-purple-600/10 rounded-full blur-3xl" />
      ...
      >
        <View style={{ marginBottom: hp(5) }}>
          <Text style={{ fontSize: rf(1.8) }} className="text-indigo-400 font-bold tracking-widest uppercase mb-2">
            Challenge Level
          </Text>
          <Text 
      ...
      {selected && (
        <View 
          ...
          className="absolute"
        >
           <StartButton label="Begin Journey" onPress={() => router.replace("/quiz")} />
        </View>
      )}
    </View>
  );
