 ...
 */
export const getLocalIpAddress = async (options: { useFallback?: boolean } = {}): Promise<string | null> => {
  try {
    ...
    if (Platform.OS === "android") {
      ...
      // 4️⃣ Fallback logic if requested or if we are clearly in a hotspot state
      if (options.useFallback || isInvalid || isCellular || netState.type === "none" || netState.type === "unknown") {
        ...
        if (options.useFallback) {
          logLanDebug(`fallback IP used: ${fallbacks[0]}`);
          updateDebugMetric("lanIsFallback", true);
          return fallbacks[0];
        }

        if (isCellular) {
          logLanDebug(`Cellular IP detected, forcing hotspot fallback: ${fallbacks[0]}`);
          updateDebugMetric("lanIsFallback", true);
          return fallbacks[0];
        }
      }
    ...
    return ip && ip !== "127.0.0.1" ? ip : null;
  } catch (e) {
